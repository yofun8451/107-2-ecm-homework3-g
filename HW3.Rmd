...
$\bar{y}_{i}=\frac{\sum_{t=1}^T y_{it}}{T}=\bar{x}_{i}'\beta+c_{i}+\frac{\sum_{t=1}^T \delta_{t}}{T}+\bar\epsilon_{i}}$
...
根據information decomposition $v_{it}$與$\alpha_{i}$獨立所以 $E(v_{it}\alpha_{i})=cov(v_{it},\alpha_i)+E(v_{it})E(\alpha_i)=0$其中因為$v_{it}$與$\alpha_{i}$獨立所以兩者covariance為0 且$E(v_{it})=0$
